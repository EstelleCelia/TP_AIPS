...
int main(int argc, char **argv)
{
    ...
    if (protocole == 1)
    {
        ...
        else
        {
            ...
            // Réception message
            if ((recvfrom(sock, mesg_recu, lg_message*sizeof(char), 0, (struct sockaddr *)&padr_em, &lg_adr_em)) == -1)
            {
    ...
    else // TCP
    {
      ...
      if(source == 1)
        ...
        //envoi du message
        if (send(sock, mesg_envoye, lg_message, 0) == -1)
        {
           printf("Erreur send\n");
            exit(1);
        }
        ...
        if (nb_message != -1)
            ...
            afficher_message(message_recu, lg_mesg_recu);
            }
            }
        else
            {
            ...
            {
            afficher_message(message_recu, lg_message);
            i++;
            }
            }
